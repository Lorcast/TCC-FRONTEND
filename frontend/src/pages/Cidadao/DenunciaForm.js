...
const DenunciaForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data: solicitacao, error: insertError } = await supabase.from("solicitacoes").insert(dadosParaSalvar).select("id").single();
      ...
      const { error: updateError } = await supabase.from("solicitacoes").update({ protocolo: numeroProtocoloGerado }).eq("id", novoId);
    ...
    <div className="min-h-screen bg-gray-100 p-6 flex items-center justify-center">
      <form onSubmit={handleSubmit} className="w-full max-w-2xl bg-white p-8 rounded-xl shadow-lg space-y-6" noValidate>
        ...
        <div>
          ...
          <textarea name="assunto" rows="1" value={formManifestacao.assunto} onChange={handleChange} className="w-full p-2 border rounded-md" />
