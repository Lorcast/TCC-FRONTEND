...
const PerfilVereadores = () => {
  ...
  const fetchVereadores = async (filtroNome = "", offsetAtual = 0) => {
    ...
    try {
      let query = supabase
        .from("vereadores")
        .select("id, nome_completo, partido, email_contato", { count: "exact" });
  ...
  // Atualiza quando offset mudar (ex: ao mudar de página)
  useEffect(() => {
    fetchVereadores(nome, offset);
  }, [offset]);

  const aplicarFiltros = () => {
    setOffset(0); // Reseta paginação ao aplicar filtros
    fetchVereadores(nome, 0);
  };

  if (carregando)
    return <p className="text-center mt-10 text-gray-600">Carregando vereadores...</p>;
